...
// ── 5 umbrales de clasificación para la leyenda de puntos (se recalculan con los datos) ──
let POINT_BREAKS = [0, 500, 1500, 3000, 6000]; // valores por defecto, se actualizan

function recalcBreaks(recintosFiltrados) {
  const vals = recintosFiltrados.map(r => r.h||0).filter(h => h > 0).sort((a,b)=>a-b);
  if (!vals.length) return;
  // Percentiles 20, 40, 60, 80
  const p = pct => vals[Math.floor(pct * (vals.length - 1))];
  POINT_BREAKS = [0, p(0.2), p(0.4), p(0.6), p(0.8)];
}

function clasePunto(h) {
  // Devuelve 0-4 según en qué franja cae
  for (let i = POINT_BREAKS.length - 1; i >= 0; i--) {
    if (h > POINT_BREAKS[i]) return i;
  }
  return 0;
}
...
function renderMapa() {
  ...
  if (document.getElementById('layerPuntos').checked && puntosOpacity > 0) {
    const zoom   = map.getZoom();
    const radius = zoom <= 6 ? 2 : zoom <= 8 ? 3 : zoom <= 10 ? 4 : zoom <= 12 ? 5 : 6;
    filtered.forEach(r => {
...
function actualizarLeyenda(paleta) {
  ...
  const el = document.querySelector('.legend-gradient');
  if (el) el.style.background = `linear-gradient(to right, ${parts.join(',')})`;
}

function actualizarLeyendaPuntos() {
  const el = document.getElementById('legendaPuntos');
  if (!el) return;
  const tValues = [0.1, 0.3, 0.5, 0.72, 0.92];
  const labels = [
    `1 – ${Math.round(POINT_BREAKS[1]).toLocaleString('es')}`,
    `${(Math.round(POINT_BREAKS[1])+1).toLocaleString('es')} – ${Math.round(POINT_BREAKS[2]).toLocaleString('es')}`,
    `${(Math.round(POINT_BREAKS[2])+1).toLocaleString('es')} – ${Math.round(POINT_BREAKS[3]).toLocaleString('es')}`,
    `${(Math.round(POINT_BREAKS[3])+1).toLocaleString('es')} – ${Math.round(POINT_BREAKS[4]).toLocaleString('es')}`,
    `> ${Math.round(POINT_BREAKS[4]).toLocaleString('es')}`
  ];
  el.innerHTML = tValues.map((t, i) => {
    const [R, G, B] = paletteColor(t, currentPalette);
    const col = `rgb(${R},${G},${B})`;
    return `<div class="lp-row">
      <span class="lp-dot" style="background:${col};box-shadow:0 0 6px ${col}88"></span>
      <span class="lp-label">${labels[i]}</span>
      <span class="lp-rank">Q${i+1}</span>
    </div>`;
  }).join('');
}
