...
function renderMapa() {
  ...
  if (document.getElementById('layerPuntos').checked && puntosOpacity > 0) {
    ...
    // Área de hit invisible: mínimo 10px para dedos/punteros, siempre mayor que el radio visual
    const hitRadius = Math.max(10, radius * 4);

    filtered.forEach(r => {
      const clase = clasePunto(r.h||0);
      const col   = colorClasePunto(clase);

      // 1. Círculo invisible de hit — captura el clic con área generosa
      const hit = L.circleMarker([r.la, r.lo], {
        radius: hitRadius,
      ...
      hit.on('click',     e => { L.DomEvent.stopPropagation(e); abrirModal(r); });
      hit.on('touchend',  e => { L.DomEvent.stopPropagation(e); abrirModal(r); });
      hit.addTo(markersLayer);

      // 2. Círculo visual pequeño — solo decorativo, no interactivo
...
function abrirModal(rec) {
  ...
  document.getElementById('modalBody').innerHTML = `
    <div class="m-stats">
      <div class="m-stat highlight"><span class="m-stat-v" style="color:#fff">${(rec.h||0).toLocaleString('es')}</span><span class="m-stat-l">habilitados</span></div>
